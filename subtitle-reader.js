...
class SubtitleReader {
  ...
  extractLinesFromHTML(htmlContent) {
    ...
    while (node = walker.nextNode()) {
      if (node.textContent.trim()) {
        // 过滤URL
        const filteredText = this.filterText(node.textContent.trim());
        if (filteredText) {
          textNodes.push(filteredText);
        }
      }
    }
  ...
  processText(text, mdHtml) {
    // 首先过滤原始文本中的URL
    const rawTextFiltered = this.filterText(text);
    
    // 从过滤后的Markdown HTML中提取行
    const mdLines = this.extractLinesFromHTML(mdHtml);
    
    // 创建清理后的文本 - 合并所有行
    const cleanedText = mdLines.join(' ');
    ...
    for (let i = 0; i < mdLines.length; i++) {
      ...
      // 智能分割这一行
      const units = this.smartSplitText(line);
      ...
      for (let j = 0; j < units.length; j++) {
        ...
        // 找到这个单位在cleanedText中的位置
        const unitPos = cleanedText.indexOf(unitText, currentPos);
        
        if (unitPos !== -1) {
          const unitEnd = unitPos + unitText.length;
